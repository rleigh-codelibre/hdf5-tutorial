...
int main()
{
  comment << "Open HDF5 file 'testc.h5' for writing" << '\n';
  auto file = H5Fcreate("testc.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);

  {
...

    {
      ...
      std::array<hsize_t, 2> max_dims = {{H5S_UNLIMITED, H5S_UNLIMITED}};
      std::array<hsize_t, 2> chunk_dims = {{4, 4}};
      comment << "Create simple dataspace (ndims=" << dims.size()
              <<")" << '\n';
      auto dataspace = H5Screate_simple(dims.size(), &dims[0], &max_dims[0]);
...

      {
        ...
        comment << "Create creation property" << '\n';
        auto cprop = H5Pcreate(H5P_DATASET_CREATE);
        comment << "Set creation property: chunk size" << '\n';
        H5Pset_chunk(cprop, chunk_dims.size(), &chunk_dims[0]);
...

        {
          ...
          comment << "Close creation property" << '\n';
          H5Pclose(cprop);
